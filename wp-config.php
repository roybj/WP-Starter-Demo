...
AUTOLOAD: {
    ...
    $debugInfo['autoload-path'] = [
        'label' => 'Autoload path',
        'value' => __DIR__ . '/vendor/autoload.php',
        'debug' => __DIR__ . '/vendor/autoload.php'
    ];
    $debugInfo['base-path'] = [
        'label' => 'Base path',
...
ENV_VARIABLES: {
     ...
     */
    $envCacheEnabled = filter_var('1', FILTER_VALIDATE_BOOLEAN);
    ...
    $phpEnvFilePath = realpath(__DIR__ . "//{$envType}.php");
...
KEYS: {
     ...
     */
    defined('AUTH_KEY') or define('AUTH_KEY', 'm3.Lu/@:k][5{Z#U ty.)w.D`@`.i~`@ +;*]$j/{J_ {(|tlGE{q7_l:&2E|;X`');
    defined('SECURE_AUTH_KEY') or define('SECURE_AUTH_KEY', '35pVu]I.Gu2$/ eN.AH{+^^~5F#o[C}|HS,$,i|,[SNLDcn&t_+o`+hY@%~aUEg&');
    defined('LOGGED_IN_KEY') or define('LOGGED_IN_KEY', '~sC{/!3y)<`#~;C.m|jKDD^]l,|C(%_7u>|C<`tg,*]pH2A@9LT==o !Kb<`cMrb');
    defined('NONCE_KEY') or define('NONCE_KEY', '/B./fMv?2W}k5(K$}[|1h*]T|<hc)#*g/Uw$Gue},F, ~hvOxFKE(<>=Nye>O{');
    defined('AUTH_SALT') or define('AUTH_SALT', 'e<eaH4d^z[UC?{+|}n0,y;J wJIo|9s+),JvJ~`%FNo`j<~<HOQlvg|o,H.P&7');
    defined('SECURE_AUTH_SALT') or define('SECURE_AUTH_SALT', 'M~`FUw#-+<bj3/Z>/U:;+Y_*nEZBfog>[FH?#6:P>u}(_&j,CLP^4|#~2bG,)m@2');
    defined('LOGGED_IN_SALT') or define('LOGGED_IN_SALT', 'CN@#LRI,:G/+:^5]R-$;bwuFr]+:j:!<zk~:X?*)%i?HwO`hU> Am*EF!r&!.eM9');
    defined('NONCE_SALT') or define('NONCE_SALT', '1_/E(e]o;I `Dt;3>)+A>]<XwH=i_MX~)i?)hYwrOp+;1!T~?1~i;,Ev6,}b&}L+');
    /**#@-*/
} #@@/KEYS
